...
namespace MusicBot2.Service
{
    public class PokeService
    {
        ...
        //所有會用到的api
        /*
         *         https://pokeapi.co/api/v2/pokemon?limit=10000&offset=0 取所有的，然後隨機挑選一筆(傻逼寶可夢會跳號)
         *         /pokemon/{id} 取單支資料
         *         /pokemon-species/{id} 取中英文名稱
         *         https://pokeapi.co/api/v2/pokemon?limit=10000&offset=0 取所有的，然後隨機挑選一筆
         *         /move/{id} 取單支招式資料
         *         
         *         
         *         testing
        ...
        public async Task<(ComponentBuilder component, Embed embed)> StartPokeGameAsync(string mode)
        {
            try
            {
                if (mode.ToLower() == "name" || mode.ToLower() == "猜pokemon名稱")
                {
                    return await StartGuessNameGameAsync();
                }
                else if (mode.ToLower() == "move" || mode.ToLower() == "猜pokemon技能")
                {
                    return await StartGuessMoveGameAsync();
                }
                else
                {
                    return CommonHelper.BuildErrorResponse("模式錯誤！請選擇 'name'(猜pokemon名稱) 或 'move'(猜pokemon技能)");
                }
            }
            catch (Exception ex)
            {
        ...
        private Embed BuildMoveEmbed(Move move, string learnedByPokemons)
        {
            ...
            {
                Title = "這哪招？",
                Description = $"**招式屬性**: 屬性:{move.damage_class?.name ?? "未知"}",
                Color = Color.Blue
            };
        ...
        public async Task<(Embed embed, ComponentBuilder component)> HandleButtonClickAsync(SocketMessageComponent interaction, int selectedId, int correctId, string correctName)
        {
            ...
            if (originalEmbed != null)
            {
                ...
                if (originalEmbed.Thumbnail.HasValue)
                {
